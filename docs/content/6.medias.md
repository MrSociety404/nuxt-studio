...
export default defineNuxtConfig({
  studio: {
    media: {
      ...
      allowedTypes: ['image/*', 'video/*', 'audio/*'] // default
    }
  }
})
```
...
::warning
External storage is only activated when **both** `media.external: true` is set **and** all five required environment variables (`S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_ENDPOINT`, `S3_BUCKET`, `S3_PUBLIC_URL`) are present. If any variable is missing, Studio falls back to default storage and logs a warning.
::
