...
::note
Studio only manages files stored under the `studio/` prefix in your bucket. Files uploaded through the Studio interface are stored as `studio/<path>` and only those files are listed, browsed, or deleted from within Studio. Any other files already in your bucket are left untouched.
::
